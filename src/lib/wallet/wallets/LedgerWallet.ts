...
export class LedgerWallet implements AbstractWallet {
    ...
    async getAccounts(): Promise<Account[]> {
        ...
        const accounts = (await signer.getAccounts()).map(x => ({
            address: x.address,
            algo: x.algo,
            pubkeyBase64: toBase64(x.pubkey),
            evmAddress: ethermintToEth(x.address),
        }))
        return accounts
    }
    ...
    async signAmino(tx: Transaction): Promise<TxRaw> {
        ...
        const pubkey = encodePubkey(encodeSecp256k1Pubkey(fromBase64(accountFromSigner.pubkeyBase64)));
